...
#eta_cr=   -0.654"
#M02s=" -0.8 -0.4 0 0.4  "
#M02s="-0.8 -0.6 -0.4 -0.2 0 0.2 0.4 0.6"
M02s="-1.2 -1.0"
mu03s="0.0120"
# 0.0186  0.0262  0.0321 
system="A3"
#rho="1"
#rho="1.008742"
rho="0"
...
if [ $system = A2 ]
then 
	echo "#SBATCH --partition=knl_usr_prod " >> $new_folder/job.templ
	echo "#SBATCH --account=INF18_lqcd123_0" >> $new_folder/job.templ
	echo "#SBATCH -N4 -n64 --mem=32000 " >> $new_folder/job.templ            
	echo "#SBATCH --time=20:00:00       " >> $new_folder/job.templ
...
#	echo "#PBS -A INF17_lqcd123_1" >> $new_folder/job.templ
	...
	echo "#SBATCH -N3 -n128 --mem=32000 " >> $new_folder/job.templ            
...
#echo "efile=/marconi/home/userexternal/rfrezzot/tmLQCD_BSM/build/contractions_BSM" >> $new_folder/job.templ
echo "efile=/marconi/home/userexternal/mgarofal/tmLQCD/build/contractions_BSM" >> $new_folder/job.templ
echo >> $new_folder/job.templ
...
if [ $system = A2 ]
then 
echo "mpirun  -n $(( 4 * 64 )) -ppn 64 \${efile} -f contractions.input  -v | tee -a eta_"$etam"_M02_"$M02_decimal"_mu03_"$mu03_decimal"_L"$L"_rho_"$rho".log" >> $new_folder/job.templ
else
echo "mpirun  -n "\$"SLURM_NTASKS  \${efile} -f contractions.input  -v | tee -a eta_"$etam"_M02_"$M02_decimal"_mu03_"$mu03_decimal"_L"$L"_rho_"$rho".log" >> $new_folder/job.templ
fi
...
#	echo "ompnumthreads=136" >>$new_folder/contractions.templ
	...
	echo "NrZPRocs = 4" >>$new_folder/contractions.templ
else
...
echo "  solverprecision = 1e-12" >>$new_folder/contractions.templ
